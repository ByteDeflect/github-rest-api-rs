...
pub async fn secret_scanning_slash_create_push_protection_bypass(configuration: &configuration::Configuration, owner: &str, repo: &str, secret_scanning_create_push_protection_bypass_request: models::SecretScanningCreatePushProtectionBypassRequest) -> Result<models::SecretScanningPushProtectionBypass, Error<SecretScanningSlashCreatePushProtectionBypassError>> {
    ...
    let local_var_uri_str = format!("{}/repos/{owner}/{repo}/secret-scanning/push-protection-bypasses", local_var_configuration.base_path, owner=crate::apis::urlencode(owner), repo=crate::apis::urlencode(repo));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());

    if let Some(ref local_var_user_agent) = local_var_configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
    local_var_req_builder = local_var_req_builder.json(&secret_scanning_create_push_protection_bypass_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<SecretScanningSlashCreatePushProtectionBypassError> = serde_json::from_str(&local_var_content).ok();
