...
pub async fn code_security_slash_detach_configuration(configuration: &configuration::Configuration, org: &str, code_security_detach_configuration_request: models::CodeSecurityDetachConfigurationRequest) -> Result<(), Error<CodeSecuritySlashDetachConfigurationError>> {
    ...
    let local_var_uri_str = format!("{}/orgs/{org}/code-security/configurations/detach", local_var_configuration.base_path, org=crate::apis::urlencode(org));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::DELETE, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CodeSecuritySlashDetachConfigurationError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
...
pub async fn code_security_slash_get_configuration_for_repository(configuration: &configuration::Configuration, owner: &str, repo: &str) -> Result<models::CodeSecurityConfigurationForRepository, Error<CodeSecuritySlashGetConfigurationForRepositoryError>> {
    ...
    let local_var_uri_str = format!("{}/repos/{owner}/{repo}/code-security-configuration", local_var_configuration.base_path, owner=crate::apis::urlencode(owner), repo=crate::apis::urlencode(repo));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CodeSecuritySlashGetConfigurationForRepositoryError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
