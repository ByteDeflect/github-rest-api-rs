...
pub async fn copilot_slash_list_copilot_seats_for_enterprise(configuration: &configuration::Configuration, enterprise: &str, page: Option<i32>, per_page: Option<i32>) -> Result<models::CopilotListCopilotSeatsForEnterprise200Response, Error<CopilotSlashListCopilotSeatsForEnterpriseError>> {
    let local_var_configuration = configuration;
...
pub async fn copilot_slash_usage_metrics_for_enterprise_team(configuration: &configuration::Configuration, enterprise: &str, team_slug: &str, since: Option<&str>, until: Option<&str>, page: Option<i32>, per_page: Option<i32>) -> Result<Vec<models::CopilotUsageMetrics>, Error<CopilotSlashUsageMetricsForEnterpriseTeamError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/enterprises/{enterprise}/team/{team_slug}/copilot/usage", local_var_configuration.base_path, enterprise=crate::apis::urlencode(enterprise), team_slug=crate::apis::urlencode(team_slug));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CopilotSlashUsageMetricsForEnterpriseTeamError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
